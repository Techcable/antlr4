...
use super::error_strategy::ANTLRErrorStrategy;

pub trait ErrorStrategyExt: ANTLRErrorStrategy {
    ///
    ...
    ///
    fn begin_error_condition(&mut self, recognizer: Parser);
    ...
    ///
    fn end_error_condition(&mut self, recognizer: Parser);
}
...
impl ErrorStrategyExt for DefaultErrorStrategy {
    #[inline]
    fn begin_error_condition(&mut self, _recognizer: Parser) {
        self.error_recovery_mode = true;
    }

    fn end_error_condition(&mut self, _recognizer: Parser) {
        self.error_recovery_mode = false;
        self.last_error_states = None;
        self.last_error_index = -1;
    }
}
impl ANTLRErrorStrategy for DefaultErrorStrategy {
    ...
    ///
    #[inline]
    fn report_match(&mut self, recognizer: Parser) {
    ...
    /// the exception
    ///
    fn report_error(&mut self, recognizer: Parser, e: RecognitionError) {
        ...
        match e {
            RecognitionError::NoViableAlt(nvae) => self.report_no_viable_alternative(recognizer, nvae),
            RecognitionError::InputMismatch(ime) => self.report_input_mismatch(recognizer, ime),
            RecognitionError::FailedPredicate(fpe) => self.report_failed_predicate(recognizer, fpe)
        }
    }
    ...
    ///
    fn recover(&mut self, recognizer: Parser, e: RecognitionError) -> Result<(), RecognitionError> {
    //		print("recover in "+recognizer.getRuleInvocationStack()+
//						   " index="+getTokenStream(recognizer).index()+
//						   ", lastErrorIndex="+
//						   lastErrorIndex+
//						   ", states="+lastErrorStates);
        if self.last_error_states.is_some()
            && self.last_error_index == getTokenStream(recognizer).index()
            && self.last_error_states.as_ref().unwrap().contains(recognizer.state()) {
...
//			errPrint("FAILSAFE consumes "+recognizer.getTokenNames()[getTokenStream(recognizer).LA(1)]);
        ...
        self.last_error_index = getTokenStream(recognizer).index()
        if self.last_error_states.is_none() {
            self.last_error_states = Some(IntervalSet::new());
        }
        self.last_error_states.as_mut().unwrap().add(recognizer.getState());
        let follow_set = getErrorRecoverySet(recognizer);
        self.consumeUntil(recognizer, follow_set)?
    }
